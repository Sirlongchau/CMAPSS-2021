...
#   TREE_THETA  -- the four the tree currently has LEAVES for (HPT/LPT eff & flow).
#   ALL_THETA   -- every component's eff/flow pair. We now EXTRACT all of them so
#                  the fan / LPC / HPC channels are on disk after a re-cache. That
#                  is what makes a unit's failure mode legible downstream: a
#                  fan-only unit has all four TREE_THETA at 0 (correct -- its HPT/LPT
#                  did not degrade), but is diagnosable only once a fan leaf exists.
#                  Extracting the fan/LPC/HPC theta is the prerequisite for two
#                  things: (1) tagging which units this HPT/LPT tree can actually
#                  read (datasets.tag_diagnosable), and (2) an observability search
#                  for fan/LPC/HPC leaves (leaf_search), exactly as was done for the
#                  four HPT/LPT modifiers. No leaf is added blindly -- a modifier is
#                  only worth a leaf if cruise-mean sensors can read it.
