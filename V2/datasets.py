...
# ==========================================================================
# From the N-CMAPSS design (Arias Chao et al. 2021). The tree only has leaves for
# HPT and LPT (TREE_COMPONENTS), so a unit is DIAGNOSABLE by this tree iff its
# failure mode degrades at least one of those two. Fan-/LPC-/HPC-only modes leave
# every TREE_THETA modifier at 0: the leaves then correctly read "no HPT/LPT
# damage", but the RUL root has NO signal to work with and can only emit
# E[RUL | hp=lp=0]. Those units are OUT OF SCOPE for an HPT/LPT damage model and
# must be scored separately -- pooling them into the headline both dilutes the
# metric and teaches the root that the (0,0) corner can mean end-of-life.
#
...
# ones) are still handled correctly -- the two must agree, and the self-test in
# datasets checks that they do.
FAILURE_MODES = {                          # subset tag -> components degraded
    "DS01":  {"HPT"},
    "DS02":  {"HPT", "LPT"},               # not in the design table given; from data
    "DS03":  {"HPT", "LPT"},
    "DS04":  {"fan"},                      # fan-only  -> UNDIAGNOSABLE by this tree
    "DS05":  {"HPC"},                      # HPC-only  -> UNDIAGNOSABLE
    "DS06":  {"LPC", "HPC"},               # LPC+HPC   -> UNDIAGNOSABLE
    "DS07":  {"LPT"},
    "DS08a": {"fan", "LPC", "HPC", "HPT", "LPT"},   # all modes -> partially readable
...
def pooled(cache=CACHE, include=None, theta=THETA, fill_missing_theta=True):
    ...
    out["failure_mode"] = out["ds"].map(
        lambda t: "+".join(sorted(FAILURE_MODES[t])) if t in FAILURE_MODES else "?")
