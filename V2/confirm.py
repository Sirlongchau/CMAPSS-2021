...
# on them (that measures a blind guess, not the damage model). We fit on the
# diagnosable units and report DIAGNOSABLE (the claim) and BLIND (the known limit)
# test units separately. `feats` carries the `diagnosable` flag from ds.pooled().
FIT_DIAGNOSABLE_ONLY = True
...
for k in range(K_SPLITS):
    ...
    if FIT_DIAGNOSABLE_ONLY:
        fitset = fitset[fitset["diagnosable"]].reset_index(drop=True)
    cap = gft.suggest_rul_cap(fitset)                       # fit-set only: no test leakage
    fit = dict(FIT_BASE, rul_cap=cap)

    scopes = [("diagnosable", test[test["diagnosable"]].reset_index(drop=True)),
              ("blind",       test[~test["diagnosable"]].reset_index(drop=True))]
    ...
    for label, tree in (DEPLOY, COMPARE):
        for s in GA_SEEDS:
            ...
            if label == DEPLOY[0] and s == GA_SEEDS[0]:
                pr = per_unit_rmse(test, m, cap)
                diag_of = test.groupby("unit")["diagnosable"].first()
                for u, r in pr.items():
                    unit_rows.append(dict(split=k, unit=int(u), rmse=float(r),
                                          diagnosable=bool(diag_of.get(u, True))))
...
print("\n(BLIND = fan/LPC/HPC-only units the tree has no leaf for. Losing to age "
      "there is expected and is the documented limit of an HPT/LPT damage model, "
      "not a regression -- it is reported, not hidden.)")
